...
import { deleteCalendarRecord, findRecordByOwner, insertCalendarRecord, updateCalendarRecord} from '$lib/schedule/schedule_sql';
...
export const actions = {
	...
	updateSchedule: async ({ locals, request }) => {
		const result = updateCalendarRecord(locals, await request.formData());

		return result;
	  },
	deleteSchedule: async({ locals, request }) => {
		const result = deleteCalendarRecord(locals, await request.formData());
