<script>
	...
	let cityInput = ""; // Input for the user to enter a city name
 
	async function getWeatherByCity() {
	  if (cityInput.trim() === "") {
		alert("도시를 입력하세요."); // Notify the user if the input is empty
		return;
	  }
 
	  try {
		const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(cityInput)}&appid=${apiKey}`);
		...
		if (data.name) {
		  location = data.name; // Update location only if the city is successfully retrieved
		}
	  } catch (error) {
  ...
  // Reset cityInput to empty when switching to current location
  cityInput = "";
  
  // Update the location variable asynchronously
  getCurrentPosition()
    .then(position => {
    ...
    .then(data => {
      ...
      getWeather(); // Trigger weather update for current location
    })
    .catch(error => {
      console.error('날씨 정보를 가져오는 중 에러 발생:', error);
      location = null;
 ...
 <style>
	...
	<div class="content">
	  ...
	  <div class="equl-line">
		<input bind:value={cityInput} placeholder="도시를 입력하세요." />
		<button on:click={getWeatherByCity}>날씨 업데이트</button>
		<button on:click={switchToCurrentLocation}>현재 위치로 돌아가기</button>
	 </div>
