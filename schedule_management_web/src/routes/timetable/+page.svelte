<script>
    ...
    function updateTimetable() {
      ...
      eventsList.forEach(event => {
        ...
        timetable[event.startHour][dayIndex].event = event.eventName;
        timetable[event.startHour][dayIndex].eventColor = event.eventColor;
  
        for (let i = 1; i < eventDuration; i++) {
          timetable[event.startHour + i][dayIndex].event = '';
          timetable[event.startHour + i][dayIndex].eventColor = event.eventColor;
        }
      });
    }
    ...
    function editEvent(hour, day) {
      const selectedEvent = eventsList.find(
        (event) =>
          event.startHour === hour && event.day === selectedDay && event.endHour > hour
      );
  
      if (selectedEvent) {
        selectedStartHour = selectedEvent.startHour;
        selectedEndHour = selectedEvent.endHour;
