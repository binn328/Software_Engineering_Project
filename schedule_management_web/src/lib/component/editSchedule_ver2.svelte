...
<div class="update-schedule-modal">
  <form method="post" action="?/updateSchedule" on:submit={handleSubmit}>
     ...
     <label>
       일정 이름
       <input name="schedule_name" type="text" required value={schedule.schedule_name} />
     </label>
     <label>
       시작일
       <input name="start_date" type="date" required value={convert_date(schedule.start_date)} />
     </label>
     <label>
       마감일
       <input name="end_date" type="date" value={convert_date(schedule.end_date)} />
     </label>
     <label>
       장소
       <input name="location" type="text" value={schedule.location} />
     </label>
     <label>
       중요도
       <input
         ...
         type="number"
         value={schedule.importance_level}
         min="1"
         max="3"
       />
     </label>
